...
2. **运营数据看板**：对话量 / token 消耗 / 裁决结果分布 / handoff 触发率 / 工具调用次数写入 KV，
   后台画图。README 讲「80% 是上线后持续调优」，有数据才立得住  
3. **RAG**（文档上传 + 检索），给 Agent 挂知识库  
4. **Handoff 携带上下文摘要**：现在转交只注入一行系统提示，目标 Agent 不知道之前聊了什么  
5. **advisor 改用 tool schema 强制结构化**：可替掉 `json_object` + 两轮质量重试，更可靠且省一次调用  
6. 更多工具（联网搜索、天气、日历）—— 加进 `api/_tools.js` 即可，无需改 chat.js  
7. Cron 结果写入 KV + 用户侧待办问候（更高频需升级 Pro 或外置调度）  
